...
Nim::ended()
{
    ...
    // calculez numărul de bile rămase
    for (int i = 0; i < 3; i++)
        count += heaps[i];

    // dacă a rămas una singură -> jucătorul care urmează la mutare a pierdut
    // dacă nu mai există nicio bilă -> jucătorul curent a pierdut
    if (count < 2)
        return true;
...
Nim::eval(int player)
{
    ...
    return heaps[1] ^ heaps[2] ^ heaps[3];
}
...
Nim::apply_move(const Move &move)
{
    ...
    if (move.heap >= 0 && move.heap < 3 && move.amount <= heaps[move.heap] && move.amount < 4 && move.amount > 0) {
...
minimax(Nim init, int player, int depth)
{
    ...
    if (init.ended() || depth == 0) {
        ...
        for (unsigned int i = 0; i < moves.size(); i++) {
            copy = init.clone();
            if (copy.apply_move(moves[i])) {
                p = minimax (copy, player, depth - 1);
                if (-p.first > score) {
                    score = p.first;
        ...
        return std::pair<int, Move>(score, move_to_make);
    }
}
...
minimax_abeta(Nim init, int player, int depth, int alfa, int beta)
{
    ...
    if (init.ended() || depth == 0) {
        ...
        for (unsigned int i = 0; i < moves.size(); i++) {
            copy = init.clone();
            if (copy.apply_move(moves[i])) {
                p = minimax_abeta (copy, player, depth - 1, -alfa, -beta);
                if (-p.first >= alfa) {
                    alfa = p.first;
                    move_to_make = moves[i];
                }
